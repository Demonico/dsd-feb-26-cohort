...
const DriverManifest = () => {
  ...
  const autoGeneratedDatesRef = useRef<Set<string>>(new Set());

  async function onGenerateRoute(dateOverride?: string) {
    const targetDate = dateOverride ?? serviceDate;
    setGenerating(true);
    setError(null);
    try {
      const data = await generateDriverManifest(targetDate);
      setManifest(data);
    } catch (e) {
  ...
  async function loadManifest(date: string) {
    ...
    try {
      ...
      if (
        date === todayIsoDate() &&
        !data.has_jobs &&
        !autoGeneratedDatesRef.current.has(date)
      ) {
        autoGeneratedDatesRef.current.add(date);
        await onGenerateRoute(date);
      }
    } catch (e) {
